...
{
    public class FabricRequisitionSlipController : ExtendedBaseController
    {
        private readonly ILogger<FabricRequisitionSlipController> _logger;
        ...
        public FabricRequisitionSlipController(IGenItemStructureGroupSubService IGenItemStructureGroupSubService,
  IGenItemStructureGroupService GenItemStructureGroupService,
           IMapper mapper, ILogger<FabricRequisitionSlipController> logger, IHttpContextAccessor contextAccessor,
        ...
        [HttpGet]
        public async Task<IActionResult> FabricRequisitionSlipLanding()
        {

            return View("~/Views/ShopFloor/FabricRequisitionSlip/FabricRequisitionSlipLanding.cshtml");
        }
        [HttpGet]
        ...
        [HttpPost]
        public async Task<IActionResult> GetFabricRequisitionSlipData(MCD_DtParameters request)
        {
            ...
            var data = (from t in records
                        select new
                        {
                            ...
                            "<button type='button' onclick='EditFabricRequisitionSlip(this)' style='font-weight: 600;background: green;color: #fff;' class='btn btn-secondary btnView'  requisition_slip_id='" + clsUtil.EncodeString(t.requisition_slip_id.ToString()) + "'>Edit Draft List</button>" +
                            "</div>"
                        }).ToList();

            var ret_obj = new AjaxResponse(records.Count > 0 ? records[0].total_rows.Value : 0, data);
            return Json(ret_obj);

        }

        [HttpPost]
        public async Task<IActionResult> GetFabricRequisitionSlipProposedData(MCD_DtParameters request)
        {
            Int64 requisitionSlipid = 0;
            int genGroupId = 1;
        ...
        [HttpPost]
        public async Task<IActionResult> GetFabricRequisitionSlipApprovedData(MCD_DtParameters request)
        {
            Int64 requisitionSlipid = 0;
            int genGroupId = 1;
        ...
        public async Task<IActionResult> FabricRequisitionSlipEdit(string requisition_slip_id)
        {
           ...
           .Select(a =>
            ...
            return PartialView("~/Views/ShopFloor/FabricRequisitionSlip/_TranFabricRequisitionSlipEdit.cshtml", model);

        }

        [HttpGet]
        public async Task<IActionResult> FabricRequisitionSlipView(string requisition_slip_id)
        {
           ...
           .Select(a =>
            ...
            return PartialView("~/Views/ShopFloor/FabricRequisitionSlip/_TranFabricRequisitionSlipView.cshtml", model);

        }

        [HttpPost]
        public async Task<IActionResult> SaveFabricRequisitionSlip([FromBody] tran_mcd_requisition_slip_DTO request)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> UpdateFabricRequisitionSlip([FromBody] tran_mcd_requisition_slip_DTO request)
        {
            var ret = true;
            ...
            try
            {
                ret = await _TranMcdRequisitionSlipService.UpdateAsync(request);
        ...
        [HttpPost]
        public async Task<IActionResult> ProposeForApproval1(Int64 Id)
        {
            try
            {
                var ret = true;
