...
$$
 ...
 RETURN QUERY
    ...
    SELECT
        ttp.techpack_number,
        ttp.techpack_date,
        ttp.tran_techpack_id,
    tdr.tran_designer_review_id,
    tpc.tran_pre_costing_id,
       ...
       tso.order_quantity,
		...
		CASE
            WHEN ttp.teckpack_style_code is not null  THEN ttp.teckpack_style_code
            ELSE tvapds.style_code
        END as style_code,
...
left outer join public.tran_tech_pack ttp on ttp.tran_designer_review_id=tdr.tran_designer_review_id
...
alter function sp_get_data_for_techpack(bigint) owner to postgres;

grant execute on function sp_get_data_for_techpack(bigint) to anon;

grant execute on function sp_get_data_for_techpack(bigint) to authenticated;

grant execute on function sp_get_data_for_techpack(bigint) to service_role;
