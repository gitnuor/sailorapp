create function sp_get_tran_range_plan_events(_fiscal_year_id bigint)
    returns TABLE(range_plan_id bigint, yearly_gross_sales numeric, tran_bp_event_id bigint, event_gross_sales numeric, readygoods_qnty bigint, readygoods_value numeric, event_id bigint, fiscal_year_id bigint, event_title text, total_range_plan_quantity bigint, total_mrp_value numeric, total_cpu_value numeric, is_finalized boolean, added_product bigint)
    language plpgsql
as
$$
DECLARE 
...
alter function sp_get_tran_range_plan_events(bigint) owner to postgres;

grant execute on function sp_get_tran_range_plan_events(bigint) to anon;

grant execute on function sp_get_tran_range_plan_events(bigint) to authenticated;

grant execute on function sp_get_tran_range_plan_events(bigint) to service_role;
