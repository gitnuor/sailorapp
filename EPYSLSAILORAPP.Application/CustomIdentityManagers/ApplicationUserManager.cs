...
{
    public class ApplicationUserManager<TUser> : UserManager<owin_user_DTO> where TUser : class
    {
        ...
        public ApplicationUserManager(
                        IUserStore<owin_user_DTO> store,
                        IOptions<IdentityOptions> optionsAccessor,
                        IPasswordHasher<owin_user_DTO> passwordHasher,
                        IEnumerable<IUserValidator<owin_user_DTO>> userValidators,
                        IEnumerable<IPasswordValidator<owin_user_DTO>> passwordValidators,
                        ILookupNormalizer keyNormalizer,
                        IdentityErrorDescriber errors,
                        IServiceProvider services,
                        ILogger<UserManager<owin_user_DTO>> logger,
            ...
            if (services != null)
            {
                foreach (var providerName in Options.Tokens.ProviderMap.Keys)
                {
                    ...
                    var provider = (description.ProviderInstance ?? services.GetRequiredService(description.ProviderType))
                        as IUserTwoFactorTokenProvider<owin_user_DTO>;
                    if (provider != null)
                    {
        ...
        public async Task<owin_user_DTO> CheckUserExists(string userName, string password, string hrprofileJsonString)
        {
            ThrowIfDisposed();
            if (userName == null)
            {
            ...
            else
            {
                ...
                //GetHRShortProfileEntity objUserHRProfile = JsonConvert.DeserializeObject<GetHRShortProfileEntity>(hrprofileJsonString);
                //objUserHRProfile.password = string.Empty;
                //objUserHRProfile.password = password;
        ...
        /// <returns></returns>
        public async Task<long> UpdateUserPasswordResetInfo(owin_user_DTO user, string code)
        {
            //ThrowIfDisposed();
        ...
        /// <returns></returns>
        public override async Task<bool> CheckPasswordAsync(owin_user_DTO user, string password)
        {
            //ThrowIfDisposed();
            //CancellationToken cancellationToken = new CancellationToken();
        ...
        public async Task<long> UserSignInLogUpdateAsync(owin_user_DTO user)
        {
        ...
        public override async Task<owin_user_DTO> FindByNameAsync(string userName)
        {
            ThrowIfDisposed();
            if (userName == null)
            {
        ...
        public async Task<owin_user_DTO> GetUserIdAsyncByUserID(string userid)
        {
            ThrowIfDisposed();
            owin_user_DTO objuser = new owin_user_DTO();
        ...
        //{
        //    //var appUser = new owin_user_DTO { emailaddress = email, username = userName };
        ...
        /// <returns></returns>
        public override async Task<IdentityResult> ResetPasswordAsync(owin_user_DTO user, string token, string newPassword)
        {
            //CancellationToken cancellationToken = new CancellationToken();
            //ThrowIfDisposed();
            //if (user == null)
        ...
        protected virtual Task<IdentityResult> UpdatePasswordHash(owin_user_DTO user, string newPassword, bool validatePassword)
            => UpdatePasswordHash(GetPasswordStore(), user, newPassword, validatePassword);

        private async Task<IdentityResult> UpdatePasswordHash(IUserPasswordStore<owin_user_DTO> passwordStore,
            owin_user_DTO user, string newPassword, bool validatePassword = true)
        {
            if (validatePassword)
            {
        ...
        private IUserPasswordStore<owin_user_DTO> GetPasswordStore()
        {
            var cast = Store as CustomUserStore;
            if (cast == null)
            {
        ...
        private IUserLockoutStore<owin_user_DTO> GetUserLockoutStore()
        {
            var cast = Store as CustomUserStore;
            if (cast == null)
            {
        ...
        public async Task<IdentityResult> SetLockoutEndDateAsync(owin_user_DTO user, DateTimeOffset? lockoutEnd)
        {
            ThrowIfDisposed();
        ...
        //    owin_user_DTO user = new owin_user_DTO();
        ...
        //public async Task<List<RefreshToken>> LoadAsyncRefreshToken(owin_user_DTO user)
        //{
        //    CancellationToken cancellationToken = new CancellationToken();
        //    if (user == null)
        //    {
