...
namespace EPYSLSAILORAPP.Infrastructure.Services
{
    ...
    public class FabricPoService : IFabricPoService
    {
        ...
        private readonly ILogger<FabricPoService> _logger;
        ...
        public FabricPoService(IGenUnitService GenUnitService, ITranScmPoService TranScmPoService, ITranPurchaseRequisitionService TranPurchaseRequisitionService, IRPCDbService RPCDbService,
             IGenItemMasterService GenItemMasterService, IGenMeasurementUnitDetailService GenMeasurementUnitDetailService,
             IGenSupplierInformationService GenSupplierInformationService, IConfiguration configuration, ILogger<FabricPoService> logger, IMapper mapper, IHostingEnvironment hostingEnvironment)
        {
        ...
        public async Task<tran_scm_po_DTO> GetSingleAsync(Int64 Id)
        {
            try
            {
               ...
               tran_scm_po_DTO po = new tran_scm_po_DTO();
                ...
                //    po.delivery_unit = pr_response.delivery_unit_id;
        ...
        public async Task<List<tran_purchase_requisition_DTO>> GetDropDownData(DtParameters param, long group)
        {
            string query = "";
            try
            {
            ...
            //           .Select("*")
            //           .Filter("pr_no", Operator.ILike, "%" + filter.Search.Value + "%")
            //           .Range(filter.Start, filter.Start + filter.Length - 1)
            //           .Where(x => x.gen_item_structure_group_id == group)
            //           .Get();
            ...
            Int64 listType)
        {
            try
            {
                using (NpgsqlConnection connection = new NpgsqlConnection(_configuration.GetConnectionString("PGConnectionString")))
                {
                    ...
                    string query = $"SELECT * FROM proc_sp_get_data_tran_scm_po_fab( @row_index,@page_size,@fiscal_year,@p_event_id,@supplier,@p_delivery_unit_id,@list_type)";
            ...
            //    var response = await _connectionSupabse.Rpc("proc_sp_get_data_tran_scm_po_fab", objFilter);
        ...
        public async Task<tran_purchase_requisition_DTO> GetPR(long pr_id)
        {
            ...
            try
            {
                ...
                //{
                //    //var delivery_unit = await _connectionSupabse.From<gen_unit_entity>().Select("*").Where(p => p.gen_unit_id == ret.delivery_unit_id).Get();
