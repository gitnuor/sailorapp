...
namespace EPYSLSAILORAPP.Infrastructure.Services
{
    ...
    public class OpenPOService : IOpenPOService
    {

        private readonly IConfiguration _configuration;
        private readonly IMapper _mapper;
		private readonly ILogger<OpenPOService> _logger;
        ...
        public OpenPOService(IGenUnitService GenUnitService, IRPCDbService RPCDbService, ITranScmPoService TranScmPoService, IGenItemMasterService GenItemMasterService, IConfiguration configuration, ILogger<OpenPOService> logger, IMapper mapper, IHostingEnvironment hostingEnvironment)
        {
        ...
        public async Task<bool> SaveAsync(tran_scm_po_entity entity, List<tran_scm_po_details_entity> details, List<file_upload> files)
        {
            try
            {
                ...
                entity.po_details = JArray.Parse(JsonConvert.SerializeObject(details.ToList())).ToString();

                //entity.is_approved = null;
                //entity.is_bill_submitted = false;
                ...
                //await _TranScmPoService.SaveAsync(entity);
                return true;
            }
            catch (Exception ex)
            {
              ...
              //  tran_scm_po_DTO ret = JsonConvert.DeserializeObject<List<tran_scm_po_DTO>>(response.Content).FirstOrDefault();
                ...
                if (DeleteFiles.Count>0)
                {
                    ...
                    if (ret.documents != null)
                    {
                        ...
                        foreach (JObject doc in ret.documents)
                        {
                            bool WillKeep = true;
                ...
                try
                {
                    ...
                    //var model = JsonConvert.DeserializeObject<tran_scm_po_entity>(JsonConvert.SerializeObject(ret));
                    //model.po_details = JArray.Parse(JsonConvert.SerializeObject(details.ToList()));
    ...
    //        }
        ...
        public async Task<List<rpc_tran_scm_po_DTO>> GetAllJoined_TranScmPoAsync(Int64 row_index, Int64 page_size, Int64 fiscal_year_id, Int64 event_id, Int64 supplier_id, long delivery_unit_id)
        {
            try
            {
                using (NpgsqlConnection connection = new NpgsqlConnection(_configuration.GetConnectionString("PGConnectionString")))
                {
                    ...
                    string query = $"SELECT * FROM proc_sp_get_data_tran_scm_po_open( @row_index,@page_size,@fiscal_year,@p_event_id,@supplier,@p_delivery_unit_id)";
            ...
            //    var response = await _connectionSupabse.Rpc("proc_sp_get_data_tran_scm_po_open", objFilter);
        ...
        public async Task<List<rpc_tran_scm_po_DTO>> GetSubmittedPOData(Int64 row_index, Int64 page_size, Int64 fiscal_year_id, Int64 event_id, Int64 supplier_id, long delivery_unit_id)
        {
            try
            {
                using (NpgsqlConnection connection = new NpgsqlConnection(_configuration.GetConnectionString("PGConnectionString")))
                {
                    ...
                    string query = $"SELECT * FROM proc_sp_get_data_tran_scm_po_submitted_open( @row_index,@page_size,@fiscal_year,@p_event_id,@supplier,@p_delivery_unit_id)";
            ...
            //    var response = await _connectionSupabse.Rpc("proc_sp_get_data_tran_scm_po_submitted_open", objFilter);
